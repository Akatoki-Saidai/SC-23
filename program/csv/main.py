...
def main():
    ...
    msg_types = ['serious_error', 'error', 'warning', 'msg', 'format_exception', 'phase', 'gnss_time', 'date', 'lat', 'lon', 'alt', 'alt_base_press', 'goal_lat', 'goal_lon', 'temp', 'press', 'camera_area', 'camera_order', 'camera_center_x', 'camera_center_y', 'camera_frame_size_x', 'camera_frame_size_y', 'motor_l', 'motor_r', 'goal_relative_x', 'goal_relative_y', 'goal_relative_angle_rad', 'goal_distance', 'accel_all_x', 'accel_all_y', 'accel_all_z', 'accel_line_x', 'accel_line_y', 'accel_line_z', 'mag_x', 'mag_y', 'mag_z', 'gyro_x', 'gyro_y', 'gyro_z', 'grav_x', 'grav_y', 'grav_z', 'euler_x', 'euler_y', 'euler_z', 'nmea']
    #先輩のコードを見てるとたまに上の行のやつにない奴が第一引数に入ってることがある(具体例は後述※2)
    #そのときは連絡ください

    #とりあえず先輩が使ってたmsg_typeをそのまま書いてるだけなのでmsg_typeを増やすこともできます
    #増やしたい時は連絡ください
    ...
    make_csv.print("warning","警告メッセージを記録すんのに使う")
